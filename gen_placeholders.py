...
out = r'D:\d\9\blog\docs\md_assets'
...
for f in os.listdir(path):
    if f.endswith('.md'):
        with open(os.path.join(path, f), 'r', encoding='utf-8') as fh:
            for m in re.finditer(r'md_assets/([^\)\s"]+)', fh.read()):
                imgs.append(m.group(1))

for img in set(imgs):
    p = os.path.join(out, img)
    if img.endswith('.jpg') or img.endswith('.jpeg'):
        open(p, 'wb').write(jpg_data)
    else:
        open(p, 'wb').write(png_data)

print(f'Created {len(set(imgs))} placeholder images')
